...
def upload_image(label, text_label):
    ...
    spoken_text = f"The color at the center is {detected_color}"
    with audio_lock:
        engine.say(spoken_text)
        engine.runAndWait()
    text_label.config(text=spoken_text)
...
def start_gui():
    window = Tk()
    window.geometry("846x762")
    window.configure(bg="#3E3688")
    ...
    Button(
        ...
        relief="flat"
    ).place(x=43.0, y=597.0, width=238.0, height=71.0)
    ...
    Button(
        ...
        relief="flat"
    ).place(x=304.0, y=597.0, width=238.0, height=71.0)
    ...
    Button(
        ...
        command=stop_video,
        relief="flat"
    ).place(x=564.0, y=597.0, width=238.0, height=71.0)
